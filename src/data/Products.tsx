...
export async function products(): Promise<Product[]> {
  ...
  const baseUrl = process.env.REACT_APP_API_URL?.replaceAll("'", "").trimEnd();
  console.log(baseUrl);
  const data = await fetchData<Product[]>(`${baseUrl}/Product`, httpMethod.GET);
  return data;
